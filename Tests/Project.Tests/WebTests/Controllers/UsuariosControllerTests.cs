...
        }
        */
        ...
        /*************************************************************************************************
         * Consultar Todos Usuários - Controller - Real
         * Teste que garante que o endpoint ConsultarTodosUsuarios retorna todos os usuários corretamente.
         **************************************************************************************************/
        
        ...
        public async Task Consultar_Todos_Usuarios()
        {
            try
            {
                ...
                var response = await _client.GetAsync($"/Usuario/ConsultarTodosUsuarios");

                response.EnsureSuccessStatusCode();

                // ⬇️ Esperamos uma lista, não um único objeto
                var usuarios = await response.Content.ReadFromJsonAsync<List<Usuario>>();

                Assert.NotNull(usuarios);
                Assert.True(usuarios!.Count > 0, "A lista de usuários está vazia.");

                foreach (var usuario in usuarios)
                {
                    _output.WriteLine($"🎯 Usuário: Nome={usuario.Nome}, Email={usuario.Email}");
                }
